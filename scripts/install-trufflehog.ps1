...
function Fail([string]$Message) {
  throw "TruffleHog installer: $Message"
}

function Publish-EnvironmentChange {
  if (-not ([System.Management.Automation.PSTypeName]"Trufflehog.EnvironmentChange").Type) {
...
"@
  ...
  $result = [UIntPtr]::Zero
  [void][Trufflehog.EnvironmentChange]::SendMessageTimeout(
    [IntPtr]0xffff,
    [uint32]0x001A,
    [UIntPtr]::Zero,
    "Environment",
...
$repo = if ($env:TRUFFLEHOG_INSTALL_REPO) { $env:TRUFFLEHOG_INSTALL_REPO } else { "trufflesecurity/trufflehog" }
# The folder goes into the user PATH, so a relative override is made absolute
# against the current location first.
$installDir = if ($env:TRUFFLEHOG_INSTALL_DIR) {
  $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($env:TRUFFLEHOG_INSTALL_DIR)
} else {
  Join-Path $env:LOCALAPPDATA "Programs\trufflehog"
}
$version = $env:TRUFFLEHOG_INSTALL_VERSION
$executables = @("trufflehog.exe")
$arch = switch ($env:PROCESSOR_ARCHITECTURE) {
  "AMD64" { "amd64" }
  "ARM64" { "arm64" }
  default { Fail "unsupported architecture: $env:PROCESSOR_ARCHITECTURE; TruffleHog ships for amd64 and arm64" }
}
...
$githubToken = if ($env:TRUFFLEHOG_GITHUB_TOKEN) {
  $env:TRUFFLEHOG_GITHUB_TOKEN
} elseif ($env:GH_TOKEN) {
  $env:GH_TOKEN
} else {
  $env:GITHUB_TOKEN
...
$apiHeaders = @{
  ...
  "User-Agent" = "TruffleHog-installer"
}
if ($githubToken) {
...
if (-not $version) {
  try {
    ...
    if ($status -eq 403 -or $status -eq 429) {
      Fail "GitHub answered HTTP $status to the latest-release lookup for $repo, its rate limit for requests without a token. Set GH_TOKEN or TRUFFLEHOG_GITHUB_TOKEN to a GitHub token, or set TRUFFLEHOG_INSTALL_VERSION to skip the lookup, then retry."
    }
    if ($status -eq 0) {
      Fail "could not reach api.github.com for the latest release of $repo. Check the network, or set TRUFFLEHOG_INSTALL_VERSION to skip the lookup, then retry."
    }
...
$archiveName = if ($env:TRUFFLEHOG_INSTALL_ARCHIVE) {
  $env:TRUFFLEHOG_INSTALL_ARCHIVE
} else {
  "trufflehog_${version}_windows_${arch}.tar.gz"
}
...
$baseUrl = if ($env:TRUFFLEHOG_INSTALL_BASE_URL) {
  $env:TRUFFLEHOG_INSTALL_BASE_URL.TrimEnd("/")
} else {
  "https://github.com/$repo/releases/download/v$version"
}

$temporaryDirectory = Join-Path ([System.IO.Path]::GetTempPath()) "TruffleHog-install-$PID"
...
# "TruffleHog 1.2.3" and a bare "1.2.3" both compare as "1.2.3".
...
try {
  Write-Output "Fetching TruffleHog $version for windows/$arch from $baseUrl..."
  Fetch $archiveName $archive
  Fetch "trufflehog_${version}_checksums.txt" $checksums
  ...
  if (-not $checksumLine) {
    Fail "trufflehog_${version}_checksums.txt has no entry for $archiveName"
  }
  ...
  if ($LASTEXITCODE -ne 0) {
    Fail "could not extract $archiveName with tar"
  }
  ...
  Write-Output "Installed trufflehog $version to $installDir"
  if (-not $env:TRUFFLEHOG_INSTALL_SKIP_PATH) {
    ...
    try {
      ...
      if ($installDir -notin $expandedEntries) {
        ...
        Write-Output "Added $installDir to your user PATH. Open a new terminal before running TruffleHog."
      }
    } finally {
