...
- A line is POSIX shell, run with `sh` on macOS and Linux, unless it carries an OS suffix, `Check (windows)`, which wins on that OS and runs in Windows PowerShell. The suffix is Go's OS name: `windows`, `darwin`, or `linux`. A command that's the same in both shells, `roast --version`, carries no suffix.
