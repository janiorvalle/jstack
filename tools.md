...
- An update runs through whoever installed the binary. `Check` is `command -v <binary>`, so setup asks the shell where the binary is and follows its links: a binary under node's global `node_modules` for an npm install line updates with that line; a link into Homebrew's Cellar gets `brew upgrade <formula>`, the formula read off the Cellar path; a file in `~/.local/bin` gets the `Install` line; anything else is shown with its path and left alone, since the install line would drop a second copy that loses on PATH, and so is a file in Homebrew's bin that isn't brew's link.
