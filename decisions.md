...
The line prompts confused: arrow keys spilled escape codes, saved picks hid a harness added later, and a Homebrew TruffleHog got an install line that put a second copy in `~/.local/bin` behind brew's on PATH. With a terminal, setup is now one screen per question on charm's huh over bubbletea, which tokenomnom already ships: harnesses every run, found ones and saved picks checked; a skills repo once; the repos folder once; per undeclared repo the backend, what it needs, the PR, and same for the rest; the tools as a checkbox list. Esc goes back a screen, Ctrl-C quits with nothing changed. The plan prints last with a confirm, and nothing in a harness or a repo of yours is written before it; the skills repo clone under `~/.jstack/repos` is still synced when the repo screen is answered, as it was, since the collisions it can raise are the next question. With nothing to apply there is no confirm, so a rerun with nothing changed is one Enter.

- One screen is one huh field in its own bubbletea program, and an answered screen collapses to one line, so the terminal reads as the list of decisions. A single program with a state machine was rejected: the shell lines run between screens with the terminal's stdin, and a program holding the tty while `gh repo clone` or a tool's version command runs is a fight over keystrokes.
- The screens only collect answers and render. `internal/setup` gained a `Session`: the facts each screen shows, cached so Esc costs nothing, and `Plan` and `Apply` for the answers. `Run`, the plan-and-flags path, goes through the same session, so every rule lives once and the no-terminal tests pass as they were. `cmd/jstack` picks the path: a terminal without `--yes` gets the screens.
...
- `internal/prompt` is gone.
- An outdated tool's update line is decided by who owns the binary, resolved with `command -v` in the shell setup uses and its links followed. An npm install line's binary is npm's only under node's global `node_modules`, from `npm prefix -g`, since node may live under Homebrew and a pnpm or hand install of the same name is nobody's. A link into `$(brew --prefix)/Cellar/<formula>/` is Homebrew's, and the formula is read off that path, so `tools.md` needs no formula line; a file in Homebrew's bin that isn't such a link is not brew's, and this Mac had one, a stray TruffleHog 3.97.0 over an unlinked 3.96.0 formula, which `brew upgrade` would have left standing. A file in `~/.local/bin` is the installer's. Anything else is shown with its path and nothing is offered. The flag path gets the same rule. A `Check` line that names no single binary, the prerequisites', can't be located and keeps the install line. `brew upgrade` that leaves the version behind the release says Homebrew is behind, not PATH.
- Windows: bubbletea runs in Windows Terminal and PowerShell, and the CI Windows job's smoke uses the flag path, so the screens aren't exercised there. The owner rule reads `(Get-Command <binary>).Source`, treats `%LOCALAPPDATA%\Programs` as the installer's folder and the npm prefix itself as npm's, where the .cmd shims sit; that path is untested on Windows.
