...
A clone or pull that fails is reported with the last line gh or git printed and setup carries on with the sources it has, the same way a failed version lookup does; a private repo gh can't reach names `gh auth status`. The clone is refreshed on every run, plan-only runs included, since the plan can't be shown without the files; that folder is jstack's own, not a harness's, so "nothing changed" still holds for the harnesses. Forgetting a repo leaves its clone on disk and its installed skills in the harnesses, now as local skills, because removing folders from a person's machine is not something a forget flag should do.
