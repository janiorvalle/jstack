...
What gh said about each origin it could see is saved in the config as `origins`, push URL to issues and when, pruned on apply to the origins the scan met. An origin gh couldn't see is asked again next run, since a failure isn't a fact. The push URL loses whatever stands before the host, because over http that is a token. `--ask-trackers-again` clears the map along with the skips. The flag path runs no git or gh and keeps the map. Issue 49's live count wraps the per-index work in `forEach`.
