...
The rule: when a vendored skill names an upstream principle that jstack already states, the jstack skill carries the upstream name as an alias in its body, and the reference stays as upstream wrote it. `strict-types` says it answers to type-system-discipline. `validate-at-the-edges` says it answers to boundary-discipline. An agent that hits the name searches the skills folder, finds the alias, and reads our version. Nothing in the vendored text changes, so the weekly bump keeps working.
