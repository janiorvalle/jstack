...
- An outdated tool's update line is decided by who owns the binary, resolved with `command -v` in the shell setup uses. npm first, by the install line, since node may live under Homebrew. Then by path: under `brew --prefix`, `brew upgrade <formula>`, the formula from a `Formula` line in `tools.md` when it isn't the binary's name; in `~/.local/bin`, the install line; anywhere else, the path is shown and nothing is offered. The flag path gets the same rule. A `Check` line that names no single binary, the prerequisites', can't be located and keeps the install line. `brew upgrade` that leaves the version behind the release says Homebrew is behind, not PATH.
- Windows: bubbletea runs in Windows Terminal and PowerShell, and the CI Windows job's smoke uses the flag path, so the screens aren't exercised there. The owner rule reads `(Get-Command <binary>).Source` and treats `%LOCALAPPDATA%\Programs` as the installer's folder; that path is untested on Windows.
