...
Releases build for macOS and Linux only. The installer is a shell script and every check and install line in `tools.md` is POSIX shell, so a Windows binary would install skills it can't check the tools for. When the stack works on Windows, the build comes back with it.
