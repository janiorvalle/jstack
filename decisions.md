...
Added 2026-09-04, for quest 420: a repo with no `Tracker:` line used to stop the agent cold, and before that agents guessed or worked untracked, so now the agent asks which backend in the Decide shape, writes the answer into the instructions file, offers a one-line PR for it on a branch named `tracker-line`, and only then claims, since one question costs less than a tracker the human never chose.
