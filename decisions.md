...
The ticket shape is enforced by a script, `scripts/ticket-lint.py`, not by prose. Problem in two sentences from the person who hits it, Fix in two or three, Done when in up to four observable lines, Out of scope optional, 120 words in all. Text asking for short tickets was already in the flow and the tickets were still walls; a lint the agent runs before filing is the second time the same correction came up, so it became a check. Files to touch and design notes go in the PR, where the reviewer reads them next to the diff.
...
Added 2026-09-04, for quest 420: a repo with no `Tracker:` line used to stop the agent cold, and before that agents guessed or worked untracked, so now the agent asks which backend in the Decide shape, writes the answer into the instructions file on its own branch named `tracker-line` in its own worktree, offers the one-line PR for it, and only then claims, since one question costs less than a tracker the human never chose.

## Setup asks where the repos live and which tracker each one uses

Written 2026-09-04, for quest 421.

Onboarding stopped at the skills and the tools: a new person still had to find out, repo by repo, how work is tracked, and every repo they own started undeclared. Now setup asks once where the repos live, guessing from the folders under home that hold git checkouts one level down, `code`, `github`, `src`, `projects`, `dev`, and saves the folders in `~/.jstack/config.json` as `repos_dirs` with a `repos_dirs_asked` flag, the same shape as the skills repo question. `--repos-dir <folder>` answers without a terminal. The plan then lists every checkout in those folders with its `Tracker:` line or `not declared`, read from `AGENTS.md`, then `CLAUDE.md`. The scan is one directory listing per folder and two small reads per checkout, no git command and no network, so a plan-only run stays as cheap as before.

The folder question is asked once and remembered. The tracker question is asked on every run for every repo that declares none, and skip is per run, on purpose: the answer lives in the repo, not the config, so a repo that shows up in the folder later gets asked the first time it appears, and a config that remembered a skip would have to be cleared by hand when the person changes their mind. With a terminal the question is a numbered pick from the four backends the `tracker` skill names, plus skip, then the one thing the backend needs, then, while other repos are still undeclared, whether the same answer is for them too. The line goes after the heading the file opens with, or first when there is none, into `AGENTS.md`, or `CLAUDE.md` when that is the only file the repo keeps, or a new `AGENTS.md` holding just the line. Without a terminal setup reports and writes nothing, since a line in someone's repo is a change to their code, not to jstack's folders.

The PR offer comes after the write, when the tree had nothing else pending and the repo has an origin: branch `tracker-line`, commit `docs: name the tracker`, `gh pr create` with a body in the ticket shape, then back to the branch the repo was on, whether or not a step after the branch failed. A dirty tree gets the line and no offer, since the commit would sweep the other changes in. Once the PR is open the checkout is back on its branch without the line, so until the PR merges the scan finds the `tracker-line` branch, as the loose ref git writes for a new branch, reports the repo as waiting on it, and doesn't ask again; the first proof run showed the question coming back on the very next run otherwise. Both checks run before the write, so the line itself never counts as the pending change. Every git and gh call goes through the same shell the tool lines use, in the checkout, so gh's login is what reaches GitHub and the binary holds no token. The remote check is `git remote get-url origin`, local and instant, rather than `gh repo view`, which would cost one API request per undeclared repo before the person has said yes to anything; a remote that isn't GitHub fails at `gh pr create` with gh's own message and the branch is left holding the line.
