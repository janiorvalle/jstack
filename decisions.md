...
A person's own skills lived only on the machine they wrote them on: one box had fifty local Codex skills, the next had none. Setup now asks once for a GitHub repo, `owner/name`, clones it with `gh repo clone` into `~/.jstack/repos/<owner>/<name>`, fast-forwards it with `gh repo sync` on every later run, and installs its `skills/` folder through the same plan as jstack's own. Both commands run through the shell setup already runs `tools.md` lines with, so the credentials stay gh's and the binary never sees a token. The first cut pulled with `git pull --ff-only`, and hung at a username prompt in the proof run: plain git has no credentials for a private https clone until someone runs `gh auth setup-git`, which a machine with gh logged in has not necessarily done. gh's own sync carries its login the way its clone does, with `--source owner/name` naming the repo itself, since without it gh syncs a fork from its parent. gh has no flag for the folder to work in, so the line changes into the clone first, `cd` with `&&` in sh and `Set-Location -ErrorAction Stop` in PowerShell, and stops there when it can't, because a sync run from whatever folder setup was started in would move that repo instead. More than one repo is fine. Only `skills/` is read: no second letter, no tools list, nothing else in the repo matters, because the letter is the one opinionated thing and two of them is the drift setup exists to prevent.

The plan is built from a list of sources, jstack's embedded folder first and then each repo, each with a name. `skills.PlanFor` takes that list, so a cloned repo's `skills/` folder is an `os.DirFS` and the same code plans and applies it. Every skill in the plan carries its source, which is how the report names the repo after each skill that isn't jstack's and how "local, untouched" stays true: a skill no source owns is local, including one a forgotten repo left behind. The other shape, one plan per source merged in setup, was rejected because every source would list the others' skills as local and setup would have to subtract.

A skill named the same in jstack and a repo, or in two repos, is a collision, and setup stops on it rather than picking quietly: with a terminal it asks, keep jstack's, use the repo's, or rename it yourself; without one it refuses and prints the `--override name=source` line that answers. The pick is saved in `~/.jstack/config.json` per skill, jstack's included, so a resolved collision never asks again, and it is printed on every later run as `overridden by owner/name` or `kept from jstack`, so the copy that isn't installed stays visible. Picks are re-derived each run: an entry whose name no longer collides is dropped on save, unless its repo couldn't be reached this run, so a clone that fails doesn't lose it. In that run the skill it names is held as installed, hidden from every source, so a lost clone plus a dead network never hands a skill back to jstack. The question about a repo is asked once and remembered even when skipped, an explicit `skill_repos_asked` in the config rather than a nil-versus-empty list, because obvious beats clever in a file people open.

A clone or pull that fails is reported with the last line gh or git printed and setup carries on with the sources it has, the same way a failed version lookup does; a private repo gh can't reach names `gh auth status`. The clone is refreshed on every run, plan-only runs included, since the plan can't be shown without the files; that folder is jstack's own, not a harness's, so "nothing changed" still holds for the harnesses. Forgetting a repo leaves its clone on disk and its installed skills in the harnesses, the ones jstack doesn't have as local skills and one that shadowed a jstack skill back to jstack's copy with a backup, because removing folders from a person's machine is not something a forget flag should do.

Three rules came out of review. A sync that fails keeps the copy from the last run: dropping the repo for that run would hand its colliding skills back to jstack, revert them with a backup, and lose the saved pick, all from a dead network. A folder named after a tool's own skill, the `Skill folder` line in `tools.md`, is left out of a repo and reported: a repo built by committing a whole skills folder carries `quest/` and `roast/` along, and the tool's own install is what keeps those matched to its binary, so setup installing the repo's copy and the tool's install overwriting it a moment later would leave the harness in a state the plan never showed. And the repo's files are read through `os.OpenRoot`, which refuses a symlink that points outside the clone, so a repo can't make setup copy a file from elsewhere on the machine into a harness. A fourth: a repo skill folder that isn't a lowercase name is left out and reported, because on the case-insensitive disks macOS and Windows ship with, `Voice` and jstack's `voice` would land in one folder while the plan treats them as two.
