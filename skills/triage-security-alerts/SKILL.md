...
Turn a stale queue of Dependabot alerts into a short list of decisions. Group first, so five advisories on one package are one decision. Recommend an action per group. The human decides. Then do exactly what was decided.

You research and recommend. You never dismiss an alert or file a task without a yes.
...
- **npm with two lockfiles and no `packageManager` in package.json.** Accidental. Each drifts on its own and Dependabot scans both, so roughly half the alerts are duplicates. Say so, offer to file a cleanup task first, then triage against the primary lockfile only and say how many alerts the cleanup will clear.
- **pip-tools with `requirements.txt` and `requirements-development.txt`, both with pip-compile headers.** Intentional. Every production package appears in both, so every advisory shows twice. Track both, dedupe by advisory, don't recommend cleanup.
...
Group by package name and scope. `runtime` is user-facing risk. `development` is build and test tooling, lower real-world impact. Pip alerts often all say runtime, so use the manifest path as a second signal.

Within a package, dedupe by advisory. One advisory across two manifests is one fix, not two. Show both numbers, "django: 24 alerts, 12 advisories". The decision is "fix this advisory", never "handle this alert id".

Sort runtime groups first, then dev-only. Inside each, by worst severity, then by count.
...
- **No upstream fix.** `first_patched_version` is null. Abandoned or free-tier libraries. Needs a replacement-library decision.
...
Same in bulk. Every `(n)` gets its grep shown before it fires.

## Filing a task

Before bundling, read the current pinned version from the lockfile and compare to the fix version. Bucket each group:
...
`not_used` is the same call with that reason. The comment is the human's words if they gave any, otherwise the template from config with the date and a reason drawn from the scope.

One decision covers every alert in the group. Fire the calls together.
