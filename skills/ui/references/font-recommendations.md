...
Good for body copy, app interfaces, and general use. Most of them work for headings too.
...
Best for headings and large display text. Many of the body/UI fonts above also work for headings. The ones here are especially strong for display, or display-only.
...
A low-contrast geometric with open apertures and a large x-height. Single-storey `a` and `g`, straight-legged `R`. Very readable at small sizes, so it's a great body font next to other headline fonts. Works for headlines too.
...
A friendly geometric with distinctive curved `t`, `f`, and `y` letterforms. Warm without being playful. Monolinear stroke. Good for friendly, approachable designs. Works for both headlines and body.
...
A geometric-humanist hybrid with open letterforms and wide proportions. The Display variant is tuned for larger sizes. Use it for headlines and display text only, never for body copy.
...
A Swiss-inspired sans-serif by Vercel. Minimal, precise, built for UI. Works for body copy, app UI, and headings.
...
A monospace font by Vercel. Good for technical or developer-oriented sites, and for code snippets and inline code.
...
A compact rationalist sans-serif with small apertures and a disciplined, closed feel. Space-efficient, so it's good for dense UI and tight layouts. Works for both headlines and body.
...
A uniwidth sans-serif. Letter widths stay the same across all weights, so changing weight never shifts layout. Good for tabs, buttons, navigation, and anywhere a weight change must not cause reflow. Works for both headlines and body.
...
A geometric neo-grotesque built from straight lines and simple circles. Uniform strokes, straight terminals. Has 12 stylistic sets for alternate glyphs. Best for clean, technical interfaces. Works for both headlines and body.
...
A high-contrast editorial serif for headlines and display text. Paired with a clean sans-serif body font it gives pages a premium, editorial feel. Works especially well for marketing sites, landing pages, and brand-forward designs.
...
A clean, highly legible sans-serif designed for screens. Works well for body copy, app UI, and headings.
...
A well-balanced contemporary serif with roots in calligraphy. Moderate contrast with subtle brush-stroke terminals. Readable at body sizes and still feels refined. Works for editorial content, blogs, and long-form reading. Works for headlines too.
...
A modernist sans-serif that blends rounded shapes with sharp angular details. The double-storey `a` and `g` give it more personality than most geometrics. Lean into that for brand-forward designs. Works for both headlines and body.
