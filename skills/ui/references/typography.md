...
Rules for text sizes, line heights, heading styles, font weights, tracking, text width, `text-pretty`, `text-balance`, and eyebrow text.
...
- Don't add `leading-*` or a line-height modifier to headings. Use Tailwind's default line-height (`text-6xl`, not `text-6xl/tight`).
- Use `text-balance` on headings and `text-pretty` on paragraphs.
- Add `tracking-tight` to headings bigger than `text-xl`, unless the font is a condensed headline font whose tracking is already tight.
- Don't use `uppercase` on eyebrow text unless it's in a monospace font. When you do, always add `tracking-wide`.
