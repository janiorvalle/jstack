...
A todo list with one entry per phase. Frame, fan out, cross-judge, pick, graft, verify. The arena runs on its own and the list is what keeps a phase from quietly disappearing.
...
2. **Write the rubric.** What does success look like for this task, turned into three to six criteria you can grade. Concrete, "adds a --dry-run flag that skips writes". Not "code is correct". The rubric is for the judge and for you. Candidates see only the task.
...
The rationale is mandatory. Without it you can't tell whether a candidate's shape is principled or accidental, and grafting gets unreliable. Each rationale names the alternatives it considered and what it rejected.
...
Read every candidate end to end before choosing. Skimming surfaces the one whose surface looks most familiar, not the best one.
...
Pick the base a future maintainer can extend most easily without breaking anything. When two feel tied, take the cleaner boundary or the smaller surface.
...
Fold each graft in by hand, as if it had been part of the design from the start. Don't paste. The result has to hold together under one mental model.

Record what was grafted, from which candidate, and what was rejected and why. The rejections are the most useful part of the record. Future readers learn from what you considered and dropped.
...
The synthesized artifact gets the same scrutiny as anything else. Run it, prove it, attach the evidence. The arena doesn't earn a pass.
