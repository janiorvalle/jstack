...
There are two kinds of decisions. Structural ones, like the shape of the data and what runs before what. And code-level ones, like how a function is written. Structural decisions are the ones that are expensive to undo. Spend your thinking there.
...
- **Data outlives code.** Your schema will be read by old code and new code at the same time, during a rolling deploy, a rollback, or an agent halfway through a task. Every schema change has to work with both. Add first, migrate, then remove. Never a change where the code and the data have to flip at the same moment.
- **Do the napkin math.** If the shape touches scale, storage, or something metered, estimate it in one line before you commit to it. Rows per day, bytes moved, dollars per month. Cheap arithmetic kills expensive designs while they're still on paper.

## At code level

Keep the types and data models consistent across the codebase. Don't try to DRY every line. Three similar statements are still better than an abstraction you don't need yet. Explicit beats clever.
...
Each change should add one coherent piece or deepen one that exists. Don't spread a new capability across callers as a pile of special cases.

Delete before you scaffold. Clear out the dead weight, then lay the foundation on what's left.
