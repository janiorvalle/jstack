...
5. **Tools.** Installs the missing tools and updates the outdated tools the human said yes to. An update runs the tool's own install line, which installs the newest release, then reads the version again. Then it runs each present tool's own skill install if its skill isn't there yet, and again for every tool it just updated, so the skill matches the binary. The tools write their skill into Claude Code's and Codex's folders, so setup then copies what they wrote into each picked harness that still lacks it, OpenCode, Cursor, or Pi, from `~/.agents/skills` when the tool put it there, else from Claude Code's folder. That copy is the tool's: the skills plan lists it as local and leaves it alone. Nothing is installed without a yes or `--install-tools`, and nothing is updated without a yes or `--update-tools`.
