...
2. **Ask.** With a terminal: once, a skills repo of your own, `owner/name`, Enter to skip. Once, where your repos live, Enter for the first folder found or a path, comma separated for more. Then a numbered harness list with the found ones preselected, then y/N per missing and per outdated tool. Without a terminal it stops here, prints the exact rerun line, and changes nothing in the harnesses, though a saved skills repo is still synced.
