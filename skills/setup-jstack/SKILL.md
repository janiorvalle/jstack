...
The plan ends with `add --skill-repo owner/name` until the skills repo question has an answer, so ask them in the same message whether they have one, and pass `--no-skill-repo` on the rerun when the answer is no. The repos section says `add --repos-dir <folder>` until that question has an answer, with the folders it found; ask where their repos live in the same message and pass `--repos-dir` on the rerun. The tracker questions need a terminal: when the section lists repos as `not declared`, tell the human, and either they run `jstack setup` themselves or you write the `Tracker:` line the way the `tracker` skill describes.
