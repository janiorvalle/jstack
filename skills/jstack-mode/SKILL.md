...
3. Check `tools.md` for anything the flow needs that isn't installed. Say so before starting, don't route around it, and get it through `jstack setup`, not the install command inside the tool's own skill.
