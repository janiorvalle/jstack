...
- **sync**. Use to install jstack on a machine or bring an install up to date. Copies the skills into your harness's skills folder, backs up anything it overwrites, never touches skills it doesn't own, then checks that the tools jstack expects are installed.
