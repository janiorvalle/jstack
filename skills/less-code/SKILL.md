...
Writing code is free for you. That's the problem. It makes it easy to add more than the job needs. So act like the person who has to maintain it and gets tired reading it. Get the most done with the least code.

## Don't add code for things that don't happen

Every guard and every extra layer is code we read, test, and keep forever. If it's protecting against something that never happens, we're paying for nothing.
...
Deleting before you add gives you a simpler base. The next change ends up smaller and less likely to break something.
...
- **Keep calls flat.** If you have to go through more than three files or layers to answer a question, flatten it. A function that hides real work behind a clean signature is fine. A chain of functions that just call the next one is not.
...
- **Leave it a little cleaner than you found it.** Rename one variable, split one function, delete one dead branch while you're in there. Aim to hand back something simpler that does the same or more.
