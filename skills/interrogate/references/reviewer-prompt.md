...
Read the surrounding code. Callers, callees, types, sibling modules. A finding that ignores what the code sits in is a bad finding.

For each finding:

1. **Severity.** `critical`, would cause bugs, data loss, security issues, or broken behavior. `warning`, a design or correctness concern that isn't broken yet but will hurt. `nit`, style or naming, only if it's useful, never to pad.
...
A good finding references specific code, explains why, separates "this is broken" from "I'd have done it differently", and accounts for the intent.
