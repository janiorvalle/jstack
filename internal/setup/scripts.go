...
func scriptsDir(home string) string {
	return filepath.Join(home, ".jstack", "scripts")
}
...
func writeScripts(embedded assets, home string) error {
	...
	if err != nil {
		return fmt.Errorf("[JSTACK-EMBED] the binary has no scripts embedded: %w; reinstall it", err)
	}
	dir := scriptsDir(home)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("[JSTACK-SCRIPTS-WRITE] cannot create %q: %w; make the home folder writable and rerun", dir, err)
	}
	for _, entry := range entries {
		content, err := fs.ReadFile(embedded.scripts, entry.Name())
		if err != nil {
			return fmt.Errorf("[JSTACK-EMBED] cannot read the embedded script %q: %w; reinstall the binary", entry.Name(), err)
		}
		path := filepath.Join(dir, entry.Name())
		if err := os.WriteFile(path, content, 0o644); err != nil {
			return fmt.Errorf("[JSTACK-SCRIPTS-WRITE] cannot write %q: %w; make its folder writable and rerun", path, err)
		}
	}
	return nil
}
